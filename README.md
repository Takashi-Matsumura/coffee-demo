...
### iPad など別端末から確認する（LAN / VPN 経由）

展示や動作確認では、iPad などから Mac の開発サーバへ LAN / VPN 経由で接続して使うことが多くなります。その際は **`next.config.ts` の `allowedDevOrigins` に接続元の IP（VPN アドレス等）を登録**してください。

```ts
// next.config.ts
const nextConfig: NextConfig = {
  allowedDevOrigins: ["100.71.3.13", "192.0.0.2"], // 接続元の IP に合わせて編集
};
```

- Next.js 16 の開発サーバは、`localhost` 以外のオリジンからの `/_next/`（JS チャンク等）取得を**既定でブロック**します。
- 未登録だと、その端末では **クライアント JS が読み込まれずハイドレーションが完了せず、`onClick` を持つボタン類が一切反応しません**（`<a>`/`<Link>` のリンクはネイティブ遷移なので動くため、「リンクは効くのにボタンだけ無反応」という紛らわしい症状になります）。
- 開発サーバのログに `Blocked cross-origin request to Next.js dev resource ... from "<IP>"` が出ていたら、その `<IP>` を `allowedDevOrigins` に追加して**開発サーバを再起動**してください。**VPN アドレスが変わったら追記が必要**です。

> iPad の Safari で「ホーム画面に追加」すると、アドレスバーの無い全画面（standalone）で起動できます（Chrome の「ホーム画面に追加」は standalone になりません）。展示で固定するなら、あわせて「設定 → アクセシビリティ → アクセスガイド」で 1 アプリにロックすると安定します。
